...
module Sessionvoc
  ...
  class Base
    ...
    def initialize(options = {})
      ...
      options.empty? ? read_configuration : self.configuration = Sessionvoc::Configuration.new(options)
      self.logger = Logger.new(STDOUT)
      self.logger.level = options['log_level']
      self.logger.info("SessionVOC Ruby Client Version: #{VERSION}")
    end
    ...
    def read_configuration
      if File.exists?("config.yml")
        ...
        self.configuration = Sessionvoc::Configuration.new(YAML.load(File.read("config.yml")))
      else
        begin
          logger.info("Using config in your home directory")
          self.configuration = Sessionvoc::Configuration.new(YAML.load(File.read("#{ENV['HOME']}/.sessionvoc.yml")))
        rescue Errno::ENOENT
          raise Sessionvoc::ConfigurationMissingException, "config.yml expected in current directory or ~/.sessionvoc.yml"
        end
      end
    end
    ...
    def handle_exception(errorCode, message = nil)
      case errorCode
        when 1
          raise Sessionvoc::InternalServerErrorException, display_message(errorCode, message)
        when 2
          raise Sessionvoc::IllegalRequestException, display_message(errorCode, message)
        when 3
          raise Sessionvoc::InvalidSidException, display_message(errorCode, message)
        when 4
          raise Sessionvoc::AuthentificationFailedException, display_message(errorCode, message)
        when 7
          raise Sessionvoc::IllegalAuthentificationRequestException, display_message(errorCode, message)
        when 8
          raise Sessionvoc::IllegalUserIdentifierException, display_message(errorCode, message)
        when 9
          raise Sessionvoc::SessionDeletionFailure, display_message(errorCode, message)
        when 11
          raise Sessionvoc::InvalidJSONException, display_message(errorCode, message)
        when 12
          raise Sessionvoc::InternalServerErrorException, display_message(errorCode, message)
        when 13
          raise Sessionvoc::InvalidFidException, display_message(errorCode, message)
        when 14
          raise Sessionvoc::FormDataCantBeModifiedException, display_message(errorCode, message)
        when 15
          raise Sessionvoc::InvalidFidException, display_message(errorCode, message)
        when 16
          raise Sessionvoc::FormDataCantBeDestroyedException, display_message(errorCode, message)
        when 17
          raise Sessionvoc::InvalidSidException, display_message(errorCode, message)
      end
    end
